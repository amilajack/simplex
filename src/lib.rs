...
impl<F: Num + Copy + PartialEq> Matrix<F> {
    ...
    pub fn set_at(&mut self, i: usize, j: usize, x: F) {
        ...
        // assert!(j < self.w);
        unsafe {
            *self.m.get_unchecked_mut(j + self.w * i) = x;
        }
    }
...
pub enum LeavingCase<F: PartialOrd> {
    NonNeg, // +infty
    Pos(usize, F),
}
...
impl<F: OrdField> Dictionary<F> {
    ...
    pub fn find_leaving_variable(&self, je: usize) -> LeavingCase<F> {
        // TODO: get rid of dumb LeavingCase :/
        use self::LeavingCase::*;
        //assert!(je != 0);
        let coeffs = (0..self.h()).map(|i| {
            if self.m.at(i, je) < F::zero() {
    ...
    pub fn find_entering_variable(&self) -> Step {
        ...
        loop {
            ...
            match fev(self) {
                Step::Continue(i, j) => {
                    if latex {
                        println!(
                            "Performing pivot: entering {}, leaving {}\n",
                            self.lc[j], self.ll[i]
                        );
                    }
